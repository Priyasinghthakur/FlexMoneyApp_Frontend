...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(
      "https://pink-hilarious-centipede.cyclic.app/check-user",
      {
        method: "POST",
        headers: {
  ...
  const submitFormData = async (formData) => {
    try {
      const response = await fetch(
        "https://pink-hilarious-centipede.cyclic.app/existing-user",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="flex-col m-4 ">
      <div className="justify-center flex">
        <div className=" align-middle text-black  justify-center flex text-3xl mb-9 bg-purple-200 w-1/4 py-4 rounded-3xl items-center">
          Book Your Slot
        </div>
      </div>
      <form className="max-w-sm mx-auto" onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          className="text-white bg-pink-300 hover:bg-pink-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
        >
          Book Slot
        </button>
      </form>
